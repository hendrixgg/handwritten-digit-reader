...
namespace std {
    ...
    Trainer::Trainer(NeuralNet* nn):  net(nn) {
        ...
        for(int l = 0; l + 1 < net->numberOfLayers; ++l) {
            weightGradient.emplace_back(net->nodesInLayer[l], vector<double>(net->nodesInLayer[l + 1]));
            biasGradient.emplace_back(net->nodesInLayer[l]);
            valueGradient.emplace_back(net->nodesInLayer[l]);
        }
    }

    void Trainer::train(const vector<vector<double>>& trainingExamples, const vector<vector<double>>& expected, const double rate) {
        if(trainingExamples.size() != expected.size()) {
            printf("ERROR: trainingExamples.size() != expected.size() (%lld != %lld)", trainingExamples.size(), expected.size());
            return;
        }
    ...
    void Trainer::backProp(const vector<double>& example, const vector<double>& expected) {
        ...
        for (int l = 0; l < net->numberOfLayers - 1; ++l) {
            for(int j = 0; j < net->nodesInLayer[l]; ++j) {
                biasGradient[l][j] += net->fPrime(net->value[l][j]) * valueGradient[l][j];
            }
        }
        ...
        for(int l = 0; l + 1 < net->numberOfLayers; ++l) {
            for(int j = 0; j < net->nodesInLayer[l]; ++j) {
                for(int k = 0; k < net->nodesInLayer[l + 1]; ++k) {
                    weightGradient[l][j][k] += net->value[l + 1][k] * net->fPrime(net->value[l][j]) * valueGradient[l][j];
                }
            }
        }
    }
