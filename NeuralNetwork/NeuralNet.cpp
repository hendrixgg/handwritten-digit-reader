...
namespace std {
    ...
    mt19937 ____generator__(chrono::steady_clock::now().time_since_epoch().count());
    ...
    void NeuralNet::setStructure(const vector<int>& dimensions) {
        ...
        for(int l = 0; l + 1 < numberOfLayers; ++l) {
            ...
            weight.emplace_back(nodesInLayer[l], vector<double>(nodesInLayer[l + 1]));
    ...
    */
    ...
    vector<double> NeuralNet::operator ()(const vector<double>& input) {
    ...
    double NeuralNet::error(const vector<double>& expected) {
